...
$(function(){
    function user_callback(context){
        ...
        if(def.module=="usercenter"&&def.action=='likeslist'){ 
           // url=def.root+"index.php?act=del&m=uc&a=like";
		url = def.root+"index.php/usercenter/dodel/act/del"
        }else if(def.module=="usercenter"&&def.action=='sharelist'){
        	url = def.root+"index.php/usercenter/delgoods"
        }else if (def.module=='album' && def.action=='details') {
		url = def.root+"index.php/album/delgoods"
	}
		...
		var aid = $(this).parent().attr('aid');
			...
			//def.root+"index.php/usercenter/likeslist/uid/"+def.uid
	                }else{ 
				alert("删除失败!");
			}
                });		
	...
	*/
        ...
        $(".like_it,.img_like_btn,.like_btn",context).click(function(){	
            ...
            var btn=$(this);
            $.post(def.root+"index.php/goods/doLike/", {
                id: iid
            }, function(data){
...
function album_callback(context){ 
    ...
    $('.img_album_btn',context).click(function(){ 
        ...
        $.post(def.root+"index.php/album/album_items_add_dialog",function(data){ 
            try{ 
           //     var error= eval("("+data+")");
                if(data=='nologin'){ 
                    login();
          ...
          //  $('.thumb',$dlg).html($('.encode_url',btn.parent().parent()).parent().html());
            $('.thumb',$dlg).html('<img style="width:100px;overflow:hidden;" src="'+$('.mysrc', btn.parent().parent()).attr('src')+'" />');
            $('.submit',$dlg).click(function(){ 
                $.post(def.root+"index.php/album/albumgoods", 
                {
                    act:'add',
                    goods_id:iid,
                    pid:$('#pid',$dlg).val(),
                    description:$('textarea',$dlg).val()
                },
                function(data){
                    if(data=='yet_exist'){ 
                        messagebox("您已经添加到该专辑了！");
                        album_items_add_dialg.close();
                        return;
                    }
		    if (data=='nologin') {
		    	window.location.href=def.root+'index.php/user/login'
		    }
                    //alert("添加成功!");
		    if (data=='done') {
	                messagebox('添加成功!');
...
$(function(){ 
    function add_follow(){ 
        $('.add_follow').click(function(){
            ...
            $.post(def.root+'index.php/usercenter/follow',{
                fans_id:fans_id,
				act:'add'
            },function(data){
		    //alert(data)
...
function login(){ 
    ...
    $.post(def.root+'index.php/user/login_dialog',function(data){ 
        art.dialog({ 
            id:'login',
            title:'登录',
            content:data,
        ...
        $('.submit',context).click(function(){ 
            ...
            $.post(def.root+'index.php/user/login',{
                name:name,
                passwd:passwd
            },function(data){ 
           
                if(data=="0"){ 					
                    $('.hint',context).html('<span style="color:red; font-weight:bold">　　用户名或密码错误</span>');
                    return;
                }
...
function get_child_cates(obj,to_id)
{
	...
	if( parent_id ){
		$.get(def.root+'index.php/usercenter/getchildcates',{parent_id:parent_id},function(data){//alert(data)
				var obj = eval("("+data+")");
