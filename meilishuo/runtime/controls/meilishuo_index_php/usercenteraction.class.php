<?php
	 ...
	 */
	class UserCenterAction extends Common {
		/**
		 * 显示用户动态
		 */
		...
		function shareList() {
			...
			$goods     = D('goods');
			$page      = new Page($goods->where(array('uid' => $uid))->total(), 30, '/uid/'.$uid);
		...
		function followList() {
			...
			$res = $user->getList($uid, $loginuid, 'fansid', 'followid');//获取关注列表
		...
		function fansList() {
			...
			$res = $user->getList($uid, $loginuid, 'followid', 'fansid');//获取粉丝列表
